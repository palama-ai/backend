...
// Ensemble models for quiz analysis and voting
...
IMPORTANT: Return ONLY valid JSON, no markdown, no explanation outside JSON.`;
  ...
  try {
    const response = await groqRequest(model, messages, { max_tokens: 500, temperature: 0.2 });
    const parsed = tryParseJsonFromText(response.text);
...
function aggregateVotes(modelResults, products) {
  const successfulResults = modelResults.filter(r => r.success && r.votes);

  if (successfulResults.length === 0) {
    console.warn('[Vote] All models failed, returning empty votes');
    return [];
  }
  ...
  successfulResults.forEach(result => {
    result.votes.forEach(vote => {
      ...
      productScores[pid].push(vote.score);
...
async function voteOnProducts({ analysis, products, options = {} }) {
  ...
  return aggregateVotes(modelResults, products);
}
