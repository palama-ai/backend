...
DO NOT include any preamble, introduction, "I appreciate", "Here is your research", or markdown code fences.
DO NOT include citations or footnotes (like [1], [2]).
If you cannot find information for a category, return an empty array for that key.
...
JSON Format:
{
  ...
  try {
    const response = await fetch(PERPLEXITY_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Strict JSON research for: ${safeName} by ${safeBrand}` }
        ],
        temperature: 0.1,
        max_tokens: 2000
      })
    ...
    let parsed = null;
    try {
      // Remove possible markdown fences first
      ...
      if (first !== -1 && last !== -1 && last > first) {
        const jsonBlock = cleaned.substring(first, last + 1);
        parsed = JSON.parse(jsonBlock);
      } else {
        // Fallback for case where no braces found but maybe it is just garbage
        console.warn('[Agent2] Perplexity returned no JSON braces. Content:', content.substring(0, 100));
        return { manufacturer: [], reputation: [], reviews: [], safety: [] };
      }
    } catch (e) {
      console.error('[Agent2] Perplexity response parsing failed:', e.message, 'Raw content start:', content.substring(0, 100));
      return { manufacturer: [], reputation: [], reviews: [], safety: [] };
    }
