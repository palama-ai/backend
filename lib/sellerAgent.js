...
Return JSON:
{
  ...
  const userMsg = `History: ${JSON.stringify(history.slice(-5))}\nUser Message: ${message || (base64Image ? "Analyzed image" : "")}`;
    ...
    try {
      ...
      // UNIVERSAL DISCOVERY: If LLM confirmed a name, but we lack details, go fetch them!
      const currentInfo = result.extracted_info || {};
      const hasName = currentInfo.name || extractedData?.name;
      const needsDiscovery = !currentInfo.description || !currentInfo.ingredients || !currentInfo.category;

      if (hasName && needsDiscovery && !discoveredData) {
        const nameToSearch = currentInfo.name || extractedData?.name;
        const brandToSearch = currentInfo.brand || extractedData?.brand || "";
        console.log(`[SellerAgent] Triggering universal discovery for: ${nameToSearch}`);
        discoveredData = await discoverProductDetails(nameToSearch, brandToSearch);
      }
      ...
      if (extractedData || discoveredData) {
        result.extracted_info = {
          ...
          category: result.extracted_info?.category || extractedData?.category || discoveredData?.category,
          price: result.extracted_info?.price || extractedData?.price || discoveredData?.suggested_price,
          ingredients: result.extracted_info?.ingredients || extractedData?.ingredients || discoveredData?.ingredients,
          barcode: result.extracted_info?.barcode || extractedData?.barcode || result.extracted_info?.barcode,
          suggested_images: discoveredData?.image_urls || result.extracted_info?.suggested_images || []
        };
        
        // Ensure image_url is set if discovered but not yet selected
        if (!result.extracted_info.image_url && discoveredData?.image_urls?.[0]) {
            result.extracted_info.image_url = discoveredData.image_urls[0];
        }
      }
