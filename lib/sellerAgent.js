...
async function discoverProductDetails(name, brand) {
  ...
  const query = `Find official product details for: "${name}" by "${brand || ''}".`;

  const systemPrompt = `You are a product research assistant. Find authentic information for the requested skincare product.
  Return ONLY valid JSON (no markdown, no extra text):
  {
    ...
    "image_urls": ["direct_image_url1.jpg", "direct_image_url2.jpg"],
    "suggested_price": "price as number only",
    "official_site": "url"
  }
  RULES:
  - If you cannot find the product, return all fields as null with empty image_urls array.
  - Do NOT include phrases like "Unable to locate" in the description - just return null.
  - image_urls must be direct image URLs ending in .jpg, .png, .webp or from known CDNs.`;
  ...
  try {
    ...
    if (!parsed) return { description: null, image_urls: [], ingredients: null, category: null };
