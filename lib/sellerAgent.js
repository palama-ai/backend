...
async function discoverProductDetails(name, brand) {
  const apiKey = process.env.PERPLEXITY_API_KEY;
  if (!apiKey || !name) return { description: null, image_urls: [], ingredients: null };

  const query = `Find the official product description, full ingredients list, and 3 high-quality direct image URLs for: "${name}" by "${brand || ''}".`;
...
Return ONLY valid JSON (no markdown):
{
  ...
  try {
    ...
    const data = await response.json();
    const content = data.choices?.[0]?.message?.content || '';
    return tryParseJsonFromText(content) || { description: null, image_urls: [], ingredients: null };
  } catch (err) {
    console.error('[SellerAgent] Discovery failed:', err.message);
    return { description: null, image_urls: [], ingredients: null };
  }
}
