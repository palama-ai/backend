...
async function searchProduct(brand, productName) {
  ...
  const [manufacturer, reputation, reviews, safety] = await Promise.all([
    braveSearch(queries.manufacturer),
    braveSearch(queries.reputation),
    braveSearch(queries.reviews),
    braveSearch(queries.safety)
  ]);
