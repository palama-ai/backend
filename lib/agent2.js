...
async function runDeepVerification(productId) {
  ...
  await sql`
    UPDATE seller_products SET
      ...
      published = ${scoringResult.verdict === 'verified' ? 1 : 0}
    WHERE id = ${productId}
  `;

  console.log(`[Agent2] ✅ Verification saved: ${scoringResult.verdict} (score: ${scoringResult.score}, visibility: ${visibilityScore}, published: ${scoringResult.verdict === 'verified' ? 1 : 0})`);
