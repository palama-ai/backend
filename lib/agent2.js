 ...
 * Uses Brave Search API + Groq LLM to verify product legitimacy,
...
const { searchProduct } = require('./braveSearch');
...
Return ONLY valid JSON, no markdown, no explanation:
{
  ...
  "sample_reviews": ["review snippet 1", "review snippet 2", "review snippet 3"],
...
async function runDeepVerification(productId) {
  ...
  try {
    ...
    if (totalResults === 0) {
      hasSearchData = false;
      console.log('[Agent2] No search results returned (API key missing or no results)');
    }
  } catch (err) {
    console.error('[Agent2] Search failed:', err?.message);
    hasSearchData = false;
    searchResults = { manufacturer: [], reputation: [], reviews: [], safety: [] };

    // Brave Search failure → set unverified with score 50
    await sql`
      ...
      WHERE id = ${productId}
    `;
    console.log('[Agent2] Set to unverified (score 50) due to search failure');
    return;
  }
  ...
  let scoringResult;
  try {
    // If no search data and no BRAVE_SEARCH_API_KEY, cap score at 60
    if (!hasSearchData) {
  ...
  console.log(`[Agent2] ✅ Verification saved: ${scoringResult.verdict} (score: ${scoringResult.score}, visibility: ${visibilityScore})`);
